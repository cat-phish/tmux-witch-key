...
main() {
	...
	tmux_echo ""
	tmux_echo "Fetching tmux keymaps..."
	...
	for window_type in "${window_types[@]}"; do
		tmux_echo "Building ${window_type^} menu..."
		...
		echo "        \"Main Menu\" \"BSpace\" \"run -b 'source \\\"\$CURRENT_DIR/witch-key.sh\\\"'\" \\" >>$WITCH_KEY_MENUS_SH
	...
	# Display a finished message to the user
	tmux_echo ""
	tmux_echo "Witch-Key build complete!"
	tmux_echo ""
	tmux_echo "Press Esc to exit"
}
